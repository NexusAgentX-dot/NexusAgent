...
export default function Dashboard() {
  ...
  const [integrationGrid, setIntegrationGrid] = useState<IntegrationStatusItem[] | null>(null)
  ...
  const activeWorkspaceAgents = workspaceAgents.filter(
    (agent) => agent.activationState === 'active' && agent.walletVerificationState === 'accepted',
  )
  ...
  useEffect(() => {
    getIntegrationStatus().then((data) => {
      if (data) {
        setIntegrationGrid(data)
      }
    })
  }, [])
  ...
  async function handleCreateAlphaRun() {
    ...
    if (!alphaRunReady) {
      setAlphaError(
        `This workspace still needs active accepted agents for: ${missingAlphaRoles
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-10">
      ...
      {backendDisconnected && (
        <p className="text-xs text-text-muted mb-4">Backend not connected &mdash; showing cached data.</p>
      )}
      ...
      {run && run.status === 'Settled' && run.settlement.txHash ? (
        ...
        <div className="rounded-xl border border-border bg-surface/40 p-4 mb-8 flex items-center gap-3">
          <div className="w-2.5 h-2.5 rounded-full bg-text-muted animate-pulse" />
          <span className="text-sm font-semibold text-text-secondary">Pending Settlement</span>
        </div>
      ) : null}
              ...
              <div className="space-y-2 text-xs">
                <div className="flex justify-between">
                  ...
                  <span className="font-mono text-text-secondary">
                    {run.payment.mode === 'x402_live' ? 'x402 via OKX Facilitator'
                      : run.payment.mode === 'transfer_event' ? 'On-chain transfer'
                      : run.payment.mode.replace(/_/g, ' ')}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-text-muted">{run.payment.label || 'Amount'}</span>
                  <span className="font-mono text-text-secondary">
      ...
      <section className="mt-10 mb-10">
        ...
        <div className="grid md:grid-cols-4 gap-3">
          {(integrationGrid ?? [
            { label: 'OKX Onchain OS', detail: 'Market + DEX + Wallet + x402', status: 'live', color: 'cyan' },
            { label: 'x402 Payment', detail: 'EIP-3009 → OKX verify → settle', status: 'live', color: 'purple' },
            { label: 'ERC-8004', detail: 'Agent Identity (mainnet)', status: 'deployed', color: 'amber' },
            { label: 'ERC-8183', detail: 'Escrow Commerce (mainnet)', status: 'deployed', color: 'amber' },
          ]).map((proto) => (
            <div
              key={proto.label}
              className={`rounded-xl border p-4 ${
                proto.status === 'live' || proto.status === 'deployed'
              ...
              <div className="flex items-center justify-between mb-2">
                ...
                <span
                  className={`text-[10px] font-mono uppercase tracking-wider ${
                    proto.status === 'live' || proto.status === 'deployed' ? `text-${proto.color}` : 'text-amber'
                  }`}
                >
                  {proto.status}
                </span>
      ...
      <section className="mt-12">
        <div className="flex items-center gap-2 mb-4">
          ...
          <span className="text-xs font-mono text-amber uppercase tracking-wider">Live Workflow</span>
        </div>
              ...
              <div className="flex items-center justify-between gap-4 mb-3">
                <div>
                  ...
                  <div className="mt-1 text-sm text-text-secondary">
                    The alpha path only runs when one active, wallet-accepted agent exists for each canonical role.
                  </div>
                </div>
                <div
                  className={`text-[10px] font-mono uppercase tracking-wider ${
                    ...
                    >
                      ...
                      <div className="mt-2 text-xs text-text-secondary">
                        {agent
                          ...
                          : 'Register and activate this role on the onboarding page.'}
                      </div>
                    </div>
                  )
                })}
            ...
            <div className="space-y-3">
              ...
              {alphaRuns.length === 0 ? (
                <div className="rounded-lg border border-border bg-abyss/50 p-4 text-sm text-text-secondary">
                  No alpha runs yet. Create one from a workspace with accepted agents.
                </div>
              ) : (
                  ...
                  <div className="grid md:grid-cols-2 gap-3">
                    ...
                    <div className="rounded-lg border border-border bg-abyss/50 p-4">
                      ...
                      <div className="mt-2 text-sm font-mono text-text-primary">
                        {selectedAlphaWorkflow.payment.mode === 'x402_live' ? 'x402 via OKX'
                          : selectedAlphaWorkflow.payment.mode === 'transfer_event' ? 'On-chain transfer'
                          : selectedAlphaWorkflow.payment.mode.replace(/_/g, ' ')} · {selectedAlphaWorkflow.payment.status}
                      </div>
                      <div className="mt-1 text-xs text-text-secondary">
