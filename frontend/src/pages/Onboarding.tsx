...
const FALLBACK_AGENTS: OnboardingAgent[] = [
  {
    ...
    walletReference: 'agentic-wallet:sentinel-demo',
  ...
  },
  {
    ...
    integrationPath: 'skills',
    walletType: 'agentic_wallet',
    walletReference: 'agentic-wallet:arbiter-demo',
  ...
  },
  {
    ...
    integrationPath: 'mcp',
    walletType: 'agentic_wallet',
    walletReference: 'agentic-wallet:executor-demo',
  ...
  },
  {
    ...
    integrationPath: 'skills',
    walletType: 'agentic_wallet',
    walletReference: 'agentic-wallet:evaluator-demo',
...
export default function Onboarding() {
  ...
  const [agentForm, setAgentForm] = useState({
    ...
    walletReference: 'agentic-wallet:builder-sentinel',
    capabilities: 'market_context',
  ...
  async function handleCreateWorkspace(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setAlphaMessage(
        `Workspace ${payload.workspace.name} is ready. Save your workspace key to reconnect from another browser or environment.`,
      )
    } catch (error) {
  ...
  async function handleVerifyAgent(agentId: string) {
    if (!selectedWorkspaceId || !selectedWorkspaceCredential) {
      setAlphaError('Select a workspace before activating an agent wallet.')
      return
    }
        ...
        <div className="grid xl:grid-cols-[1.1fr_0.9fr] gap-6">
          <div className="rounded-xl border border-border bg-surface/40 p-6">
            ...
            <div className="mt-4 rounded-lg border border-border/70 bg-abyss/40 p-4">
              ...
              <p className="text-xs text-text-secondary mt-1 mb-4">
                Workspaces are keyed. Paste a workspace id and its workspace key to reconnect this browser without listing every workspace publicly.
              </p>
          ...
          <div className="rounded-xl border border-border bg-surface/40 p-6">
            ...
            <p className="text-sm text-text-secondary mb-5">
              This is the first real step from showcase to product: an outside builder can create a workspace-scoped agent record and move it from draft to accepted.
            </p>
      ...
      <section className="mb-16 grid md:grid-cols-2 gap-4">
        <div className="rounded-xl border border-cyan/20 bg-cyan/[0.04] p-6">
          ...
          <p className="text-sm text-text-secondary mb-3">
            ...
            See the CLI Quick Start below for install and setup commands.
          </p>
        </div>
...
const result = await client.runWorkflow(ws.workspaceId, 'Check OKB and execute')`}</pre>
      ...
      <section className="mt-16">
        ...
        <p className="text-sm text-text-secondary mb-6">
          These are the real builder-facing records. Agents begin in <span className="font-mono">draft</span>, then move to <span className="font-mono">active</span> once the wallet reference is accepted (format validation + best-effort Onchain OS reachability check).
        </p>
                ...
                <div key={agent.agentId} className="rounded-lg border border-border bg-surface/30 p-4">
                  ...
                  <div className="mt-4 flex items-center justify-between gap-3">
                    ...
                    <button
                      ...
                      disabled={isAlphaBusy || agent.walletVerificationState === 'accepted'}
                    ...
                    >
                      {agent.walletVerificationState === 'accepted' ? 'Accepted' : 'Activate'}
                    </button>
                  </div>
                </div>
              ))
