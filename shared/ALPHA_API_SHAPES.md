...
## Purpose
This file defines the first API surface required for the builder onboarding layer.
...
### Accept wallet reference for agent
...
- `workflowTemplateId`
- `intent`
- `requestedAgentIds` (optional when the server should auto-select one active accepted agent per canonical role)
...
### Rule 1A
All workspace-scoped endpoints must require a workspace key.

### Rule 2
Workflow execution endpoints must reject agents whose wallet is not `accepted` or whose activation is not `active`.

### Rule 3
History endpoints must never return proof or usage from another workspace.

### Rule 4
The demo API may continue to exist alongside the workspace API, but workspace endpoints are the preferred product API.
