...
export interface TestnetFlowArtifact {
  workflowId: string
  heroUseCase: string
  chain: {
    name: string
  ...
  settlementMode: 'live_testnet_transfer'
  sender: string
  recipient: string
  token: {
    address: string
...
async function writeArtifactFiles(artifact: TestnetFlowArtifact) {
  const __filename = fileURLToPath(import.meta.url)
  const __dirname = path.dirname(__filename)
  const repoRoot = path.resolve(__dirname, '../../..')
  const outputDir = path.join(repoRoot, 'output', 'testnet-flow')
...
export async function executeBoundedTestnetTransfer(input: {
  workflowId: string
  heroUseCase: string
  ...
  const privateKey = requireEnv('NEXUSAGENT_XLAYER_TEST_PRIVATE_KEY')
  ...
  if (network.chainId !== DEFAULT_CHAIN_ID) {
    throw new Error(
      `Unexpected chain id ${network.chainId.toString()}. Expected ${DEFAULT_CHAIN_ID.toString()} for X Layer testnet.`,
    )
  }
  ...
  try {
    ...
    if (!receipt || receipt.status !== 1) {
      throw new Error('Bounded testnet transfer failed or receipt was not confirmed.')
    }
    ...
    const artifact: TestnetFlowArtifact = {
