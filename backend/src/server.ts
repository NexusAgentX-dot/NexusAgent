...
app.get('/api/health', (_req, res) => {
  ...
  res.json({
    ...
    integrations: {
      ...
      xlayerTestnet: !!(process.env.NEXUSAGENT_XLAYER_TEST_PRIVATE_KEY),
      mcpServer: true,
    },
  ...
  if (!process.env.NEXUSAGENT_ALLOW_PUBLIC_WORKSPACE_LIST) {
    res.status(403).json({
      error: {
        ...
        message: 'Workspace listing is disabled for private builder alpha.',
  ...
  if (!workflowRun) {
    res.status(404).json({
      error: {
        ...
        message: `No proof payload found for workflow id ${id}`,
        hint: 'Use run_demo_001 or run_demo_rejected_001 in demo mode.',
      },
    })
    return
...
import { buildPaymentRequirement, validatePaymentHeader, buildPremiumSignal } from './integrations/x402.js'
...
app.get('/api/integrations/status', (_req, res) => {
  res.json({
    ...
    x402: {
      ...
      paymentVerification: 'simplified',
    },
    agenticWallet: {
...
app.get('/api/signals/premium-okb', async (req, res) => {
  ...
  // Validate payment
  const validation = validatePaymentHeader(paymentHeader)
  if (!validation.valid) {
    res.status(402).json({
      error: {
        code: 'PAYMENT_INVALID',
        message: validation.reason,
        hint: 'Base64-encode a JSON object with txHash or signature field.',
      },
    })
    return
  }

  // Payment accepted → return premium signal
  try {
    const premium = await buildPremiumSignal(validation.txHash)
    res.json(premium)
  } catch (error) {
    res.status(502).json({
      error: {
...
app.get('/.well-known/agent.json', (req, res) => {
  res.json({
    ...
    provider: {
      ...
      url: 'https://github.com/nexusagent',
    },
