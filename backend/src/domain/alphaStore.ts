...
type AlphaStoreState = {
  ...
  workflows: Array<{
    ...
    payment: {
      mode: 'x402_live' | 'transfer_event' | 'x402_compatible_demo'
...
async function readState() {
  ...
  const state = alphaStoreSchema.parse(JSON.parse(raw))

  // Normalize legacy state values on read
  for (const agent of state.agents) {
    if ((agent.walletVerificationState as string) === 'verified') (agent as Record<string, unknown>).walletVerificationState = 'accepted'
    if ((agent.walletVerificationState as string) === 'unverified') (agent as Record<string, unknown>).walletVerificationState = 'pending'
    if ((agent.activationState as string) === 'verified') (agent as Record<string, unknown>).activationState = 'active'
  }
...
export async function registerAgentForWorkspace(
  ...
  const agent = alphaAgentRecordSchema.parse({
    ...
    walletVerificationState: 'pending',
    createdAt: timestamp,
    updatedAt: timestamp,
  })
...
export async function verifyAgentWalletForWorkspace(workspaceId: string, agentId: string) {
  ...
  // Real wallet reachability check via Onchain OS
  let walletReady = false
  let walletNote = 'Wallet check not available'
  try {
    ...
    walletNote = walletCheck.note
    console.log(`[Alpha] Wallet check for ${agentId}: ready=${walletReady} note=${walletNote}`)
  } catch (err) {
    console.log(`[Alpha] Wallet check failed for ${agentId}: ${err instanceof Error ? err.message : err}`)
  }

  // Validate wallet reference format (Ethereum address or OKX wallet URI)
  const walletAddr = state.agents[agentIndex].walletReference
  const isEthAddress = /^0x[0-9a-fA-F]{40}$/.test(walletAddr)
  const isOkxWallet = walletAddr.startsWith('okx://')
  if (!isEthAddress && !isOkxWallet) {
    throw new Error(`Invalid wallet reference format: ${walletAddr}`)
  }

  // Wallet acceptance state machine (format validation + best-effort reachability):
  // - Onchain OS check passed → accepted + active
  // - Onchain OS check failed but valid format → accepted + active (graceful: API may be unavailable)
  // - OKX wallet URI (no on-chain check possible) → accepted + active
  // - Invalid format → rejected above
  let verificationState: 'accepted' | 'draft'
  if (walletReady) {
    verificationState = 'accepted' // Real check passed
  } else if (isOkxWallet) {
    verificationState = 'accepted' // OKX wallet URI — check delegated to OKX infrastructure
  } else if (isEthAddress) {
    verificationState = 'accepted' // Valid address, Onchain OS check unavailable — graceful pass
  } else {
    verificationState = 'draft'
  }
  const activationState = verificationState === 'accepted' ? 'active' : 'draft'

  const updatedAgent = alphaAgentRecordSchema.parse({
    ...state.agents[agentIndex],
    walletVerificationState: verificationState,
    activationState: activationState,
    updatedAt: nowIso(),
  })
  console.log(`[Alpha] Agent ${agentId} wallet: ${verificationState} (${walletNote})`)
