...
import { executeBoundedTransfer } from '../live/executeBoundedTransfer.js'
...
// Ensure NEXUSAGENT_XLAYER_NETWORK is consistent with execution mode
if (ALPHA_EXECUTION_MODE === 'mainnet_transfer' && !process.env.NEXUSAGENT_XLAYER_NETWORK) {
  process.env.NEXUSAGENT_XLAYER_NETWORK = 'mainnet'
}
...
export async function createAlphaWorkflow(input: {
    ...
    : REQUIRED_ALPHA_ROLES.map((role) => {
        const agent = input.availableAgents.find(
          (candidate) =>
            ...
            candidate.walletVerificationState === 'accepted',
        )
  ...
  if (requestedAgents.some((agent) => agent.walletVerificationState !== 'accepted')) {
    throw new Error('All requested agents must have accepted wallets before a workflow can be created.')
  }
  ...
  if (approved && (ALPHA_EXECUTION_MODE === 'testnet_transfer' || ALPHA_EXECUTION_MODE === 'mainnet_transfer')) {
    ...
    try {
      ...
      workflowSteps[2] = {
        ...
        inputSummary: `Bounded ${ALPHA_EXECUTION_MODE === 'mainnet_transfer' ? 'mainnet' : 'testnet'} execution enabled.`,
        outputSummary: verification.ok
          ? `Submitted bounded ${ALPHA_EXECUTION_MODE === 'mainnet_transfer' ? 'mainnet' : 'testnet'} transfer ${artifact.txHash}.`
      ...
      settlement = {
        ...
        proofSummary: verification.ok
          ? `Alpha workflow produced a bounded live X Layer ${ALPHA_EXECUTION_MODE === 'mainnet_transfer' ? 'mainnet' : 'testnet'} settlement artifact for ${artifact.amount} ${artifact.token.symbol}.`
      ...
      proofArtifact = {
        ...
        workspaceId: input.workspaceId,
        chain: 'X Layer',
        network: ALPHA_EXECUTION_MODE === 'mainnet_transfer' ? 'mainnet' : 'testnet',
        txHash: artifact.txHash,
        explorerUrl: artifact.explorerUrl,
      ...
      workflowSteps[2] = {
        ...
        role: executor.role,
        status: 'failed',
        inputSummary: `Bounded ${ALPHA_EXECUTION_MODE === 'mainnet_transfer' ? 'mainnet' : 'testnet'} execution was attempted for the alpha workflow.`,
        outputSummary: '',
        startedAt: executionStartedAt,
        completedAt: failedAt,
