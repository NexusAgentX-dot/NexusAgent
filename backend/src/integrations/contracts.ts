...
const PRIVATE_KEY = process.env.NEXUSAGENT_XLAYER_TEST_PRIVATE_KEY?.trim() || ''
const RPC_URL = process.env.NEXUSAGENT_XLAYER_RPC_URL?.trim() || 'https://testrpc.xlayer.tech/terigon'
...
export async function runEscrowJobLifecycle(params: {
  ...
  try {
    ...
    // createJob — client is deployer, agent is also deployer (self-service demo)
    console.log(`[ERC-8183] Creating escrow job...`)
    const createTx = await escrow.createJob(
      jobId,
      signer.address,  // agent = deployer for testnet demo
      params.tokenAddress,
      params.amount,
      params.intentHash,
    )
    ...
    // For testnet demo, we skip funding if token balance is insufficient
